...
def complete_task():
    try:
        ...
        conn.commit(); cur.execute("SELECT points FROM users WHERE id = %s", (uid,))
        score = cur.fetchone()[0]; cur.close(); conn.close()
...
def buy_miner():
    try:
        ...
        if res and res[0] >= 10000:
            ...
            conn.commit(); cur.execute("SELECT points FROM users WHERE id = %s", (uid,))
            new_score = cur.fetchone()[0]; cur.close(); conn.close()
            return jsonify({"success": True, "new_score": new_score})
        cur.close(); conn.close(); return jsonify({"success": False, "message": "Solde insuffisant"})
    except Exception as e: return jsonify({"success": False, "message": str(e)}), 500
...
def tap():
    try:
        ...
        res = cur.fetchone(); miners = res[0] if res else 0
        ...
        cur.execute("SELECT points FROM users WHERE id = %s", (uid,))
        score = cur.fetchone()[0]; conn.commit(); cur.close(); conn.close()
...
if __name__ == '__main__':
    init_db(); app.run(host='0.0.0.0', port=10000)
